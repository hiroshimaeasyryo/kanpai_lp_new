/**
 * contents-manager 保存用 API（Google Cloud Functions Gen2）
 * worksLp の gcp-save-api と同様の契約。PAT は環境変数で1回だけ設定し、編集者はアクセスコードのみで保存可能。
 *
 * 環境変数: GITHUB_TOKEN, CONTENTS_SAVE_SECRET
 * デプロイ後、client/public/repo-config.json の saveApiUrl / saveApiSecret に設定する。
 */
...
exports.saveContent = (req, res) => {
  ...
  if (!path || content === undefined || !owner || !repo || !GITHUB_TOKEN) {
