...
- **複数LP運用**: 一つのドメインで複数のLPを配信する構成を採用している。URL はトップ `/` のほか、`/main`・`/campaign2603` などのスラグで別LPを表示する。ドメイン数を増やさずにキャンペーン別LPを増やせる。
...
| ID | 要件 | 詳細 | 優先度 |
|----|------|------|--------|
| F-17 | スラグ別URL | 同一ドメイン配下で、トップ `/` および `/{スラグ}`（例: `/main`, `/campaign2603`）で異なるLPコンテンツを表示する。 | 必須 |
| F-18 | 編集対象の選択 | Contents Manager で、編集するLPを一覧から選択し、選択したLPのコンテンツのみを読み込み・保存する。 | 必須 |
...
| スラグ | 複数LP運用で各LPを識別する名前。URL の `/{スラグ}` およびコンテンツファイル `content/{スラグ}.json` に使う。予約語（contents-manager, content, root 等）は使用しない。 |
